...
import { FileDoesNotExistError } from "../../src"
import { FileReader } from "../../src/node"
...
describe("FileReader", () => {
    ...
    describe("Constructor", () => {
        ...
        test("should throw FileDoesNotExistError for non-existent file path", () => {
            expect(() => new FileReader({ file: "/non/existent/file.txt" })).toThrow(FileDoesNotExistError)
        })

        test("should throw FileDoesNotExistError for directory path", () => {
            expect(() => new FileReader({ file: tempDir })).toThrow(FileDoesNotExistError)
        })
    })
