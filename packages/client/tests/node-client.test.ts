...
beforeAll(() => {
    if (!process.env.FILELIB_API_KEY || !process.env.FILELIB_API_SECRET || !process.env.FILELIB_API_BASE_URL) {
...
describe("Node Client addFile", () => {
    ...
    test("addFile with valid file path must succeed", () => {
        ...
        const metadata = { name: "test", size: 1000 }

        expect(() => client.addFile({ file: filePath, metadata })).not.toThrow()
    ...
    test("addFile with non-existent file must throw FileDoesNotExistError", () => {
        ...
        expect(() => client.addFile({ file: filePath, metadata })).not.toThrow()
        // Note: The error is caught internally and passed to onError callback
    })
    ...
    test("addFile with string config must create new Config instance", () => {
        ...
        client.addFile({ file: filePath, config: "custom-storage", metadata })
        expect(client.files).toHaveLength(1)
    })
    ...
    test("addFile with Config instance must use provided config", () => {
        ...
        client.addFile({ file: filePath, config: customConfig, metadata })
        expect(client.files).toHaveLength(1)
    })
    ...
    test("addFile without config must use client's config", () => {
        ...
        client.addFile({ file: filePath, metadata })
        expect(client.files).toHaveLength(1)
    })
    ...
    test("addFile with onError callback must handle errors gracefully", () => {
        ...
        client.addFile({ file: filePath, metadata, onError })
    ...
    test("addFile must handle multiple files correctly", () => {
        ...
        client.addFile({ file: file1, metadata: { name: "file1" } })
        client.addFile({ file: file2, metadata: { name: "file2" } })
